...
import { Car, LayoutDashboard, CarFront, CalendarCheck, MapPin, MessageSquare, FileText, LogOut, Menu, X, ChevronRight, UserCog, Users, Sun, Moon } from 'lucide-react';
...
export default function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    api.get('/api/auth/me').then(r => {
      ...
      if (data.mustChangePassword && pathname !== '/admin/changer-mdp') {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex">
          ...
          {NAV_ITEMS.map((item) => (
            <Link
              ...
              className={cn(
                ...
                isActive(item.href) ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-800'
              )}
            >
      ...
      <div className="flex-1 min-w-0">
        <header className="sticky top-0 z-30 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 h-14 flex items-center px-4 gap-4">
          ...
          <div className="ml-auto flex items-center gap-3">
            <button onClick={() => setTheme(resolved === 'dark' ? 'light' : 'dark')} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
              {resolved === 'dark' ? <Sun className="h-4 w-4 text-amber-400" /> : <Moon className="h-4 w-4 text-gray-500" />}
            </button>
